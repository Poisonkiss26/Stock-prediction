            ...
            if self is result:
                ...
                for child in self.get_children():
                    if child.value is not None:
                        ...
                        # print(np.linalg.norm(self.jacobi))
        return self.jacobi
...
class Variable(Node):
    ...
    def __init__(self, dim, init=False, trainable=True, **kargs):
        ...
        if hasattr(init, '__iter__'):
            if init[0] == 'Gaussian':
                self.value = np.mat(np.random.normal(init[1], init[2], self.dim))
            else:
                raise 'The initialization of the node value get incorrect format.'
        else:
            self.value = np.mat(np.random.normal(0, 0.05, self.dim))
        # 变量节点是否参与训练
