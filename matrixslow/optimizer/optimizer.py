...
class Optimizer(object):
    ...
    def __init__(self, graph, target, learning_rate=0.01, printgrad=False):
        """
...
class Adam(Optimizer):
    ...
    def __init__(self, graph, target, learning_rate=0.01, beta_1=0.9, beta_2=0.99, printgrad=False):
    ...
    def _update(self):
        ...
        for node in self.graph.nodes:
            if isinstance(node, Variable) and node.trainable:
                ...
                # 取得该节点在当前批的平均梯度
                gradient = self.get_gradient(node)
                value = np.linalg.norm(node.value)
                if self.printgrad:
                    print(f'{node.name}:\t{np.round(value,6)}', end='\n')
